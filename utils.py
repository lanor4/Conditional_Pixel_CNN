...
def antoine_to_one_hot(tensor, n, fill_with=1.):
    one_hot = torch.zeros(tensor.size(0), n)  # Adjust the size to be B x n directly
    if tensor.is_cuda:
        one_hot = one_hot.cuda()
    one_hot.scatter_(1, tensor.unsqueeze(1).long(), fill_with)  # Ensure tensor is long and add unsqueeze for scatter_
    return one_hot 
